...
generate_scenarios <- function(start_age, end_age) {
  ...
  coverages <- c(1)  # Coverage varies from 0.4 to 1
...
df_prop <- df[!is.na(df$value),] %>%
  count(group, value) %>%
  group_by(group) %>%
  mutate(prop = n / sum(n))
# Plot proportion with facet_wrap for a 2x5 layout, sorted properly
colnames(df) <- c("value", "interval")
df_prop$interval <- as.numeric(gsub("Interval_", "", df_prop$group))
# 
# ggplot(df_prop, aes(x = value, y = prop, fill = factor(interval))) +
#   geom_col(position = "dodge") +
#   scale_fill_manual(values = colors) +
#   theme_minimal() +
#   labs(x = "Number of Screening Events Between 30-49", y = "Proportion of Women", fill = "Intervals") +
#   theme(
...
#     axis.text = element_text(face = "bold", size = 14),  # Bold and increase axis text font
...
#     plot.margin = margin(0, 0, 0, 0)  # No extra margins
#   ) +
#   
